...
st.header('2-2. Line chart')
st.line_chart(df)
st.line_chart(df['A'])

st.header('2-3. Area Chart')
st.area_chart(df.A)
st.area_chart(df)

st.area_chart(df, y=['A','B','C'], color=['#ff0033', '#009933', '#000066'])

st.header('2-4. Scatter plot')
...
st.header('2-6. matplotlib pyplot')
...
folium.Choropleth(geo_data=geo_gg,
                  data=df_gg[2007],
                  coliumns=[df_gg.index, df_gg[2007]],
